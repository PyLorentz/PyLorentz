...
from scipy.ndimage.interpolation import rotate as rotate2d
...
def sim_images(mphi, ephi, Dshp=None, del_px = 2, def_val = 200_000, add_random = False, path = None, b0=1e4):
    ...
    altem.defocus = 0.0
    im_in = altem.getImage(ObjWave,qq,del_px)
    altem.defocus = -def_val
    im_un = altem.getImage(ObjWave,qq,del_px)
    altem.defocus = def_val
    im_ov = altem.getImage(ObjWave,qq,del_px)
...
def std_mansPhi(mag_x, mag_y, shape=None, del_px = 1, b0=1e4):
    ...
    if shape is None:
        ...
        thk_map = shape 
    altem = Microscope(E=200e3,Cs = 200.0e3, theta_c = 0.01e-3, def_spr = 80.0)
    ephi = altem.sigma * (thk_map * isl_V0 + np.ones(mag_x.shape) * mem_thk * mem_V0)
    
    return (ephi, mphi)

# ================================================================= #
#                                                                   #
#          Various functions for displaying images and such         #
#                                                                   #
# ================================================================= #
